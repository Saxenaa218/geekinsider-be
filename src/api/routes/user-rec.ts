...
export default (app: Router) => {
   ...
   */
  route.post('/user', middlewares.isAuth, middlewares.attachCurrentUser, middlewares.isCompany, async (req, res) => {
  ...
  route.get('/search-can', middlewares.isAuth, middlewares.attachCurrentUser, async (req: Request, res: Response) => {
    const logger: Logger = Container.get('logger');
  ...
  route.get('/getcan', middlewares.isAuth, middlewares.attachCurrentUser, async (req: Request, res: Response) => {
    const logger: Logger = Container.get('logger');
   ...
   */
  route.get('/applied', middlewares.isAuth, middlewares.attachCurrentUser, async (req, res) => {
    const logger: Logger = Container.get('logger');
    try {
